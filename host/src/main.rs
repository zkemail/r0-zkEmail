...
fn generate_and_verify_proof<T, U>(
    ...
    -> Result<U>
    where T: Serialize, U: DeserializeOwned + Debug
{
    ...
    receipt.verify(*verify_id).map_err(|e| anyhow!("Failed to verify proof: {}", e))?;
...
async fn main() -> Result<()> {
    ...
    if args.len() < 3 {
        return Err(anyhow!("Usage: {} <from_domain> <email_path> [regex_config_path]", args[0]));
    }
    ...
    let config_path = if args.len() > 3 { Some(PathBuf::from(&args[3])) } else { None };

    match config_path {
        Some(config) => {
            info!("Starting email verification with regex config from {:?}", config);
            verify_email(
                from_domain,
                &email_path,
